...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (token && !user) {
      try {
        ...
        const u = { id: p.id, userId: p.userId, name: p.fullName || p.name || p.email?.split('@')[0], email: p.email, role: p.role };
        setUser(u);
        syncDoctorId(u);
      } catch { logout() }
  ...
  const login = (newToken, userData = null) => {
    ...
    let u;
    if (userData) {
      u = { id: userData._id || userData.id, userId: userData.userId, name: userData.fullName || userData.name, email: userData.email, role: userData.role };
    } else {
      const p = JSON.parse(atob(newToken.split('.')[1]))
      u = { id: p.id, userId: p.userId, name: p.fullName || p.email?.split('@')[0], email: p.email, role: p.role };
    }
    setUser(u);
    syncDoctorId(u);
  }
