...
import { 
  ...
  Stethoscope, CreditCard, Video, CalendarDays
} from 'lucide-react'
...
const Sidebar = ({ isPatient, isDoctor, isAdmin }) => {
  ...
  const doctorLinks = [
    ...
    { name: 'Patient Files', icon: FileText, path: '/doctor-reports' },
    { name: 'Settings', icon: User, path: '/profile' },
  ]
  ...
  const adminLinks = [
    ...
    { name: 'Revenue', icon: CreditCard, path: '/admin/payments' },
  ]
      ...
      <nav className="flex-1 px-4 space-y-1">
        {links.map((link) => (
          <Link
            key={link.name}
            to={link.path}
            className={`flex items-center justify-between group px-4 py-3 rounded-2xl text-[14px] font-bold transition-all duration-200 ${
              isActive(link.path)
          ...
          >
            <div className="flex items-center gap-3">
              <link.icon 
                size={18} 
                className={`transition-colors ${isActive(link.path) ? 'text-medigo-blue' : 'text-slate-400 group-hover:text-medigo-navy'}`} 
              />
              {link.name}
            </div>
            {isActive(link.path) && (
              <motion.div layoutId="activeInd" className="w-1.5 h-1.5 rounded-full bg-medigo-blue" />
            )}
          </Link>
        ))}
      </nav>
